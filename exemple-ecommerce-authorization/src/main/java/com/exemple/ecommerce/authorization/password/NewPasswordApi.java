...
public class NewPasswordApi {
    ...
    public Response post(@NotNull @Valid NewPassword newPassword, @NotBlank @HeaderParam(FeatureConfiguration.APP_HEADER) String app) {
        ...
        loginResource.get(newPassword.getLogin()).ifPresent((JsonNode login) -> {
            ...
            String accessToken = JWT.create().withArrayClaim("scope", new String[] { "login:update", "login:read" }).withClaim("id", id.toString())
                    .withExpiresAt(expiresAt).withClaim("client_id", securityContext.getAuthentication().getOAuth2Request().getClientId())
